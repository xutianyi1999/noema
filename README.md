...
Noema 是由 OpenCode 驱动的文本知识库服务：提交 `.md`/`.txt` 文档，Agent 将其编译成 LLM-WIKI 风格的知识节点并构建知识图谱，之后的自然语言查询基于库内证据作答、携带**经服务端逐字核验的引用**（原文引文 + 字符偏移 + 条号定位，可直接用于前端高亮）。面向法规、合同、公文、标准指南等权威文档场景设计，通用文本同样适用。
...
- **证据可追溯**：节点声明来源；回答以契约 JSON 携带引用，服务端逐字核验引文、计算偏移与行范围、映射回知识节点——代码只做与文档结构无关的通用校验（子串、字符计数），条号等语义地址由 Agent 声明；任务与查询留有可审计历史。
...
sequenceDiagram
    ...
    opt 多节点枚举 / 跨概念关系 / 先摸清相关内容版图
        A->>G: 加载上游 graphify Skill 做图谱导航（只读）
        G-->>A: 相关节点与关系
    end
    A-->>N: 契约 JSON 答案（answer + references：source/quote/locator[/start/end]）
    N->>N: 逐字核验引文 ∈ 原文 · 验算或重算字符偏移 · 清洗失效 [n] 标记
    N-->>U: 答案 · 核验过的引用（引文/偏移/行范围/条号/对应知识节点）
```

查询全程不写任何知识文件；session 用完即销毁，只在控制面留下查询历史。查询契约（JSON Schema、字段要求、生成示例）与摄入纪律由 Rust 类型经 `schemars`/serde 生成，幂等写入库项目的 `AGENTS.md`，经 OpenCode 注入该库**每个 session 的 system prompt**——用户问题与摄入任务描述作为 user 消息原样进会话，不掺指令文本；服务启动时按库刷新，旧库自动收敛到当前二进制的契约版本。应答由 `jsonschema` 在服务端按同一份 schema 校验，与 system prompt 同源、不会漂移：Agent 的最终答案须是 `<noema-answer>` 标记内的契约 JSON。引用只受理 `raw/` 原始文档——wiki 节点是编译出的摘要，只用于导航，结论经由节点找到时沿其 `sources` 回溯到原文再引用。每条引用以 `quote`（逐字引文）为锚：引文不是来源文件的真实子串即整条丢弃，`[n]` 标记随之清除（不重新编号，未过核验的论断以无引用形式保留）；`start/end` 偏移 Agent 给了就验算（`content[start..end] == quote`）、没给或不对就由服务端重算，连同 1 基行范围一并返回。`locator`（第三十三条、5.2.1 之类）是 Agent 按来源自身编号声明的人类地址，服务端原样透传、不做文档结构解析；可验证的事实只有引文与偏移。原文引用同时挂上对应知识节点（若存在），形成"原文 → 节点"两级追溯。答案不是合法契约 JSON 时降级为纯文本答案 + 空引用并记日志——未核验的引用不如没有。
