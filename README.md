...
查询全程不写任何知识文件；session 用完即销毁，只在控制面留下查询历史。查询契约（JSON Schema、字段要求、生成示例）与摄入纪律由 Rust 类型经 `schemars`/serde 生成，幂等写入库项目的 `AGENTS.md`，经 OpenCode 注入该库**每个 session 的 system prompt**——用户问题与摄入任务描述作为 user 消息原样进会话，不掺指令文本；服务启动时按库刷新，旧库自动收敛到当前二进制的契约版本。应答由 `jsonschema` 在服务端按同一份 schema 校验，与 system prompt 同源、不会漂移：Agent 的最终答案须是 `<noema-answer>` 标记内的契约 JSON。每条引用以 `quote`（逐字引文）为锚：引文不是来源文件的真实子串即整条丢弃，`[n]` 标记随之清除（不重新编号，未过核验的论断以无引用形式保留）；`start/end` 偏移 Agent 给了就验算（`content[start..end] == quote`）、没给或不对就由服务端重算，连同 1 基行范围一并返回。`locator`（第三十三条、5.2.1 之类）是 Agent 按来源自身编号声明的人类地址，服务端原样透传、不做文档结构解析；可验证的事实只有引文与偏移。原文引用同时挂上对应知识节点（若存在），形成"原文 → 节点"两级追溯。答案不是合法契约 JSON 时降级为纯文本答案 + 空引用并记日志——未核验的引用不如没有。
